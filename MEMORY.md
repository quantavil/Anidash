...
- [2026-06-14] Visual and Ergonomic Upgrades → ROOT CAUSE: Layout, contrast, and ergonomic flaws identified in audit (clipped detail rows, missing mobile safe-area padding, low contrast badges, invisible empty stars, redundant icons). Also, score rating stars were too small/numerous, and episodes did not auto-complete when marked as Completed. → FIX: Replaced pb-SAFE with pb-safe (defined via env), changed watching badge text color for high contrast, redesigned RatingStars to 5 larger stars supporting half-stars, updated empty stars to text-white/20, formatted character names as First Last, wrapped metadata rows, and set episodes to total on completed status.
- [2026-06-14] Detail Page Layout Redesign → ROOT CAUSE: Mobile horizontal scroll navigation tabs were clipped and had poor scroll indicators. → FIX: Removed tab pills entirely, converting the page to a single scrollable feed in the order of Overview, Related, Recommendations, and Characters. Added Synopsis and Characters expand/collapse buttons to keep the layout compact, and formatted character lists as compact grids.
