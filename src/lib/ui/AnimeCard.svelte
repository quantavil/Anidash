<script lang="ts">
  ...
  const imageUrl = $derived(
    entry.mainPicture?.large ?? entry.mainPicture?.medium ?? null,
  );
</script>
...
>
  <div class="flex flex-col h-full overflow-hidden rounded-[14px] bg-surface-1 shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]">
    <!-- Cover Image -->
    <div class="relative aspect-[3/4] w-full overflow-hidden bg-surface-2 border-b border-border">
      ...
      <!-- User score overlay -->
      {#if entry.score > 0}
        <div class="glass-badge absolute bottom-2 right-2 px-2 py-0.5 text-xs font-bold border-primary/30 bg-primary/20">
          <Star size={10} class="mr-1 text-warning" fill="currentColor" />
          {entry.score}
        </div>
      {/if}
