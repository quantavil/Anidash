<script lang="ts">
	...
	import { formatMediaType, formatRelativeDate, formatSeason } from '$lib/utils/format';
	...
	const progressPct = $derived(
		entry.numEpisodes > 0 ? Math.min((entry.numWatchedEpisodes / entry.numEpisodes) * 100, 100) : 0
	);
</script>
	...
	<div class="min-w-0 flex-1">
		...
		>
			...
			{#if dubStore.hasDub(entry.malId)}
				<span
					class="inline-flex ml-2 align-middle items-center justify-center shrink-0 rounded-md border border-primary/20 bg-primary/10 px-1 py-0.5 text-primary"
					title="Dubbed"
				>
