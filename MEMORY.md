...
- [2026-05-02] Audit Finding: Accessibility & Performance in UI → ROOT CAUSE: Missing aria-labels in FilterBar and TabBar, lazy loading above-the-fold images, and inconsistent glass styling on skeletons/error pages. → FIX: Applied `aria-label` attributes, conditionally applied `loading="eager"` via `index` prop in ImageWithFallback, and centralized `.glass-card` CSS utility.
- [2026-05-02] Audit Finding: Missing TTLs and security hardening in Auth and Sync → ROOT CAUSE: Incomplete defense-in-depth measures. → FIX: Added `state` CSRF token to OAuth flow with 5-minute TTL, implemented 7-day TTL and exponential backoff for the offline sync queue, configured strict CSP headers, and enforced build-time environment variable checking for `VITE_MAL_CLIENT_ID`.
...
- [2026-05-30] NSFW Content Filtering → ROOT CAUSE: Search query forced `sfw: true` filter on all requests, preventing mature content from displaying. → FIX: Removed `sfw: true` query parameter from search API payload.
- [2026-05-30] PWA Startup Latency (5-6s) → ROOT CAUSE: Blocking network requests to MAL profile (`/users/@me`) on init stalled layout rendering. → FIX: Implemented optimistic caching of user profile in localStorage and switched to background revalidation.
