...
describe('userlist.svelte.ts state', () => {
	...
	it('should queue failed deletions to the sync queue', async () => {
		...
		vi.mocked(deleteAnimeStatus).mockResolvedValueOnce({ ok: false, error: { type: 'api', status: 500, message: 'Server error' } });
		...
		expect(putSyncQueue).toHaveBeenCalledWith(expect.objectContaining({
			malId: 4,
