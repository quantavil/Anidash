<script lang="ts">
  ...
  async function doSearch(page: number = 1, append: boolean = false) {
    ...
    const result = await searchAnime({
      ...
      order_by: filterSort === 'popularity' ? 'members' : filterSort === 'title' ? 'title' : filterSort === 'start_date' ? 'start_date' : 'score',
...
<div class="mx-auto max-w-7xl px-4 py-6">
    ...
    <div class="mt-3 rounded-xl border border-border bg-surface-1 p-4">
      <div class="flex flex-wrap gap-4">
        ...
        <div class="min-w-[140px]">
          <label for="filter-type" class="mb-1.5 block text-xs font-medium text-text-muted">Type</label>
          <select
            id="filter-type"
            value={filterType}
        ...
        <!-- Genre -->
        <div class="min-w-[140px]">
          <label for="filter-genre" class="mb-1.5 block text-xs font-medium text-text-muted">Genre</label>
          <select
            id="filter-genre"
            value={filterGenre}
        ...
        <!-- Sort -->
        <div class="min-w-[140px]">
          <label for="filter-sort" class="mb-1.5 block text-xs font-medium text-text-muted">Sort By</label>
          <select
            id="filter-sort"
            value={filterSort}
  ...
  {#if filterType || filterGenre}
    <div class="mt-3 flex flex-wrap gap-2">
      {#if filterType}
        <span class="inline-flex items-center gap-1 rounded-full bg-primary/10 px-3 py-1 text-xs text-primary">
